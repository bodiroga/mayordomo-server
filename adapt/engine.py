...
class IntentDeterminationEngine(pyee.EventEmitter):
    ...
    def __get_unused_context(self, parse_result, context):
        ...
        final_context = []
        final_context_category = {}
        for context in result_context:
            category = context["data"][0][1]
            if category not in final_context_category:
                final_context.append(context)
                final_context_category[category] = context["confidence"]
            else:
                if context["confidence"] > final_context_category[category]:
                    for c in final_context:
                        if c["data"][0][1] == category:
                            final_context.remove(c)
                            break
                    final_context.append(context)
                    final_context_category[category] = context["confidence"]
    ...
    def register_entity(self, entity_value, entity_type, alias_of=None, metadata=None):
        """
        ...
        """
        if alias_of:
            self.trie.insert(entity_value.lower(), data=(alias_of, entity_type, metadata))
        else:
            self.trie.insert(entity_value.lower(), data=(entity_value, entity_type, metadata))
            self.trie.insert(entity_type.lower(), data=(entity_type, 'Concept', metadata))
