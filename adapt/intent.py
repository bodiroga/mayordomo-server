...
def find_first_tag(tags, entity_type, after_index=-1):
    for tag in tags:
        for entity in tag.get('entities'):
            if len(entity.get('data')[0]) == 2:
                entity.get('data')[0] += (None,)
            for v, t, m in entity.get('data'):
...
class Intent(object):
    ...
    def validate_with_tags(self, tags, confidence):
        ...
        result['missing_required_tag'] = []
        result['entities'] = {}
        intent_confidence = 0.0
        local_tags = tags[:]
        used_tags = []

        required_points, optional_points, at_least_points = 0, 0, 0
        max_required_points, max_optional_points, max_at_least_points = 0, 0, 0

        for require_type, attribute_name in self.requires:
            max_required_points += 1
            required_tag, canonical_form, confidence = find_first_tag(local_tags, require_type)
            if not required_tag:
                result["missing_required_tag"].append(attribute_name)

            else:
                result['entities'][attribute_name] = canonical_form
        ...
        for optional_type, attribute_name in self.optional:
            ...
            optional_points += 0.5 * conf

        maximum_points = max_required_points + max_optional_points + max_at_least_points
        real_points = required_points + optional_points + at_least_points

        total_confidence = real_points / maximum_points
